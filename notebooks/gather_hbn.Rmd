...
hbn_joelle <- read_csv(here(data_path, "data-2022-03-29T21_15_06.086Z.csv")) %>%
  slice(-1) %>%
  rename("participant_id" = Identifiers) %>%
  mutate(participant_id = str_remove(participant_id, ",assessment")) %>%
  mutate(participant_id = str_c("sub-", participant_id))
...
```{r}
hbn_ <- list(hbn_raw, hbn_lei, hbn_joelle) %>%
  reduce(., dplyr::full_join, by = 'participant_id')
```
...
```{r}
hbn_ <- hbn_ %>%
  mutate(age = as.numeric(`Basic_Demos,Age.x`))
```
...
hbn_ <- hbn_ %>%
  mutate(sex.x = case_when(
    `Basic_Demos,Sex.x` == "0"    ~ "Male",
    `Basic_Demos,Sex.x` == "1"    ~ "Female",
    TRUE                        ~ "Other"
  ) %>% factor(levels = sex_levels))

hbn_ <- hbn_ %>%
  mutate(sex.y = case_when(
    `Basic_Demos,Sex.y` == "0"    ~ "Male",
    `Basic_Demos,Sex.y` == "1"    ~ "Female",
    TRUE                        ~ "Other"
  ) %>% factor(levels = sex_levels))

conflict_sex <- hbn_ %>% filter(sex.x != sex.y)

stopifnot(nrow(conflict_sex) == 0)


hbn_ <- hbn_ %>% mutate(sex = sex.x)
```
...
```{r}
hbn_ %>%
  select(`PreInt_Demos_Fam,Child_Race.x`) %>%
  table()

hbn_ %>%
  select(`PreInt_Demos_Fam,Child_Race.y`) %>%
  table()
```
...
hbn_ <- hbn_ %>%
  mutate(race_old = case_when(
    `PreInt_Demos_Fam,Child_Race.x` == "0"  ~ "White/Caucasian",
    `PreInt_Demos_Fam,Child_Race.x` == "1"  ~ "Black/African American",
    `PreInt_Demos_Fam,Child_Race.x` == "2"  ~ "Hispanic",
    `PreInt_Demos_Fam,Child_Race.x` == "3"  ~ "Asian",
    `PreInt_Demos_Fam,Child_Race.x` == "4"  ~ "Indian",
    `PreInt_Demos_Fam,Child_Race.x` == "5"  ~ "Native American Indian",
    `PreInt_Demos_Fam,Child_Race.x` == "6"  ~ "American Indian/Alaskan Native",
    `PreInt_Demos_Fam,Child_Race.x` == "7"  ~ "Native Hawaiian/Other Pacific Islander",
    `PreInt_Demos_Fam,Child_Race.x` == "8"  ~ "Two or more races",
    `PreInt_Demos_Fam,Child_Race.x` == "9"  ~ "Other race",
    `PreInt_Demos_Fam,Child_Race.x` == "10" ~ "Unknown",
    `PreInt_Demos_Fam,Child_Race.x` == "11" ~ "Other",
  ) %>% factor()
)
#0= White/Caucasian
...
```{r}
hbn_ <- hbn_ %>%
  mutate(education_years.x=as.numeric(`PreInt_EduHx,yrs_school.x`))
hbn_ <- hbn_ %>%
  mutate(education_years.y=as.numeric(`PreInt_EduHx,yrs_school.y`))

conflict_edu <- hbn_ %>% filter(education_years.x != education_years.y)

stopifnot(nrow(conflict_edu) == 0)


hbn_ <- hbn_ %>% mutate(education_years = education_years.x)
```
...
hbn_final <- hbn_ %>%
  mutate(wave = "1",
         study = "HBN" %>% factor(),
         Site = `Basic_Demos,Study_Site.x` %>% factor()
  ) %>%
  mutate(study_site = case_when(Site=="1" ~ "HBN-1",
                                Site=="2" ~ "HBN-2",
                                Site=="3" ~ "HBN-3",
                                Site=="4" ~ "HBN-4",
                                Site=="5" ~ "HBN-5")) %>%
  select(participant_id, study, study_site, wave, age, sex, race, bmi, handedness, education_years, education_years_f=education_f, education_years_m=education_m)
```
