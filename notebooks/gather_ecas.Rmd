...
conflict_edu <- ecas_ %>% filter(education_years.x != education_years.y)

stopifnot(nrow(conflict_edu) == 0)


ecas_ <- ecas_ %>% mutate(education_years = education_years.x)
```

```{r}
ecas_$education_years %>%
  table()
```
...
```{r}
ecas_ %>%
  mutate(education_f = Father_Edu, education_m = Mother_Edu) -> ecas_
...
ecas_final <- ecas_ %>%
  ...
  mutate(study_site = case_when(Site=="1" ~ "ecas-1",
                                Site=="2" ~ "ecas-2",
                                Site=="3" ~ "ecas-3",
                                Site=="4" ~ "ecas-4",
                                Site=="5" ~ "ecas-5")) %>%
