...
```{r}
pnc_final <- pnc_ %>%
  mutate(wave = timepoint %>% as.character(),
         study = "PNC",
         selection = "Community"
  ) %>%
  mutate(study_site = "PNC-1") %>%
  select(participant_id, study, study_site, wave, age, sex, race, handedness, education_years, education_years_f = education_f, education_years_m = education_m, bmi)
```
...
education_years_f <- list(
  LongName = "FatherEducation",
  Description = "Father's education in years at time of scan/intake.",
  Levels = levels(pnc_final$education_f)
)

education_years_m <- list(
  LongName = "MotherEducation",
  Description = "Mother's education in years at time of scan/intake.",
